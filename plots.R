...
## true parameter is wrong for effprop, repprop, effprop*repprop
Tpara <- function(x){
  if(x=="R0"){return(R0)}
  if(x=="effprop"){return(effprop)}
  if(x=="repprop"){return(repprop)}
  if(x=="Effprop*rep"){return(effprop*repprop)}
}
...
hline.data <- data.frame(z = c(effprop,R0,repprop), parameter = c("effprop","R0","repprop"))
